...
const SetVariantGallery = () => {
  store.variants[props.index].gallery = [...store.variants[props.index].gallery, ...selectedImages.value]
  ...
  <div class="variant-select-existing" v-if="showProductGallery">
    ...
    <div class="wrapper">
      <div class="content shadow-md">
        ...
        <div class="thumbs" v-if="store.product.gallery">
          <div
            v-for="(image, j) in store.product.gallery.filter(
              (img) => !store.variants[index].gallery.find((item) => item._id == img._id)
            )"
            :key="image._id"
            class="thumb shadow-md relative"
            :class="{
              selected: store.variants[index].gallery.find((img) => img._id == image._id),
            }"
            :ref="(el) => (galleryThumbRef[j] = el)"
            @click="setSelectedImages(image, j)"
          >
            <div>
              <!-- <div v-if="!store.variants[index].gallery.find((img) => img._id == image._id)"> -->
