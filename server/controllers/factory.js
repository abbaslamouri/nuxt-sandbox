...
const getDoc = async (Model, id) => {
  ...
  if (!doc) return next(new AppError(`We can't find a document with ID = ${id}`, 404))
  return doc
}
...
const deleteDocuments = async (Model, params, modelName) => {
  let data = null
  try {
    if (params.id) {
      data = await Model.findByIdAndDelete(params.id)
      if (!data) {
        const newError = new Error(
          `Unknown error while deleting document with ID = ${params.id} from Model ${modelName}`
        )
      ...
      data = await Model.deleteMany(params)
      if (!data) {
        const newError = new Error(
