const useAttributeTerm = () => {
  ...
  const fetchAll = async (params) => {
    ...
    try {
      const response = await $fetch(`/api/v1/attributeterms`, { params })
  ...
  const fetchBySlug = async (slug) => {
    ...
    state.value.message = null
    try {
      const response = await $fetch(`/api/v1/attributeterms`, { params: { slug } })
      // console.log(response)
  ...
  const saveDoc = async (doc) => {
    state.value.errorMsg = null
    state.value.message = null
    // console.log('KKKKKK'attributeterms
    try {
      if (doc._id) {
        ...
        await $fetch(`/api/v1/attributeterms`, { method: 'POST', body: doc })
      }
...
export default useAttributeTerm
